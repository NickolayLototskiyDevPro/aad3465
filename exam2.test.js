...
describe('Dev-Pro OpenJS - Exam 2', () => {
  ...
  describe('Method removeParticipant', () => {
    ...
    /**
     * Test was fixed.
     * We have to use different participant objects in order to test this case, but we set up participant and
     * try to remove participant with the same seniorityLevel - intermediate. And it removes. But in this case should
     * test opposite case.
     */
    it('It shouldn\'n remove any participant and execute callback with null on removeParticipant method call', (done) => {
      const participants = [{ seniorityLevel: 'middle' }]; // before fix was [{ seniorityLevel: 'intermediate' }]
      project.init(participants, {});
      project.removeParticipant(participant, (removedParticipant) => {
        expect(project.participants).toEqual(participants);
